console.log('Boller3D Server v1.0.135'); 
let express; 
try { 
...
/** Spawn Python for project scripts. Uses PYTHON_PATH / B3D_PYTHON, else Windows `py -3`, else `python`. */
function spawnPythonScript(scriptPath, scriptArgs, spawnOptions = {}) {
    const env = {
        ...process.env,
        PYTHONUTF8: '1',
    ...
    const opts = { ...spawnOptions, env };
    const custom = process.env.PYTHON_PATH || process.env.B3D_PYTHON;
    if (custom) {
        return child_process.spawn(custom, [scriptPath, ...scriptArgs], opts);
    }
    if (process.platform === 'win32') {
        try {
            child_process.execFileSync('py', ['-3', '-c', 'import sys; sys.exit(0)'], { stdio: 'ignore', timeout: 8000 });
            return child_process.spawn('py', ['-3', scriptPath, ...scriptArgs], opts);
        } catch (_) { /* use python below */ }
    }
    return child_process.spawn('python', [scriptPath, ...scriptArgs], opts);
}

function pythonVersionProbeLog() {
    const custom = process.env.PYTHON_PATH || process.env.B3D_PYTHON;
    const candidates = custom
        ...
        : process.platform === 'win32'
            ? [['py', ['-3', '-V']], ['python', ['-V']]]
            : [['python3', ['-V']], ['python', ['-V']]];
    for (const [exe, vargs] of candidates) {
        try {
            const pyVersion = child_process.execFileSync(exe, vargs, { encoding: 'utf8', timeout: 8000 }).trim();
            const pyPath = child_process.execFileSync(exe, [...(exe === 'py' ? ['-3'] : []), '-c', 'import sys; print(sys.executable)'], { encoding: 'utf8', timeout: 8000 }).trim();
            console.log(`Node.js will use Python Version: ${pyVersion}`);
            console.log(`Node.js will use Python Path: ${pyPath}`);
            return;
        } catch (_) { /* try next */ }
    }
    throw new Error('No working Python found (tried py -3 / python). Set PYTHON_PATH to your python.exe.');
}
...
app.post('/api/repair-stl', async (req, res) => {
    ...
    // Expecting { stlData: "<STL string>", holeSize?: number }
    const stlDataString = req.body.stlData; 
    const holeSizeRaw = req.body.holeSize;
    const parsedHoleSize = Number.parseFloat(holeSizeRaw);
    const holeSize = Number.isFinite(parsedHoleSize) ? Math.max(0.0, parsedHoleSize) : 100.0;
    ...
    try {
        ...
        const pythonProcess = child_process.spawn(pythonExecutable, pythonArgs, { env: { ...process.env, B3D_FAST: '1', B3D_STRICT: '1' } });
...
app.post('/api/subtract-stl', async (req, res) => {
    ...
    // Expecting { modelStlData: "<STL string>", logoStlData: "<STL string>", preserveTopology?: boolean }
    const { modelStlData, logoStlData, preserveTopology = false } = req.body;
...
app.post('/api/subtract-stl-scripted', async (req, res) => {
    ...
    try {
        ...
        // Force STRICT mode for final-logo intersection so per-body intersections are preserved
        // and aggressive post-repair steps do not collapse/discard smaller components.
...
app.post('/api/intersect-stl-scripted', async (req, res) => {
    ...
    try {
        ...
        // 2. Intersection via subtract_script (pre-repair seals open logos so Manifold works).
        // overlap_script.py skips that repair and fails on typical browser extrusions ("Not all meshes are volumes!").
        const scriptPath = path.join(__dirname, 'subtract_script.py');
        
        try {
            console.log('Checking Python executable used by Node.js (PYTHON_PATH / py -3 / python)...');
            pythonVersionProbeLog();
        } catch (pyCheckError) {
            console.error('Failed to locate Python:', pyCheckError.message);
            return res.status(500).json({ error: 'Python executable not found or failed to execute.', details: pyCheckError.message });
        }
        ...
        console.log(`Executing Find Overlap (subtract_script intersection): script=${scriptPath} args=${pythonArgs.join(' ')}`);

        const pythonProcess = spawnPythonScript(scriptPath, pythonArgs, {});
        ...
        const timeoutMs = 300000; // subtract_script intersection: multi-body repair can be slow
        const to = setTimeout(() => {
            if (!pythonProcess.killed) {
                killedByTimeout = true;
                console.error(`Python intersection timed out after ${timeoutMs}ms. Killing process.`);
        ...
        pythonProcess.on('close', async (code) => {
           ...
           console.warn(`Input file cleanup disabled for debugging: ${modelInputPath}, ${logoInputPath}`);
            ...
            if (killedByTimeout) {
                ...
                return res.status(504).json({ error: 'Python intersection timed out.' });
            }
            if (code !== 0) {
                // Log the FULL error message
...
app.post('/api/analyze-stl', async (req, res) => {
    try {
        const { stlData } = req.body || {};
        if (!stlData) return res.status(400).json({ error: 'Missing stlData' });

        const tempDir = path.join(__dirname, 'temp_analyze');
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir);
        const inputPath = path.join(tempDir, `analyze_${Date.now()}.stl`);
        await fs.promises.writeFile(inputPath, stlData, 'utf8');

        const pythonExecutable = 'python';
        const scriptPath = path.join(__dirname, 'analyze_script.py');
        const args = [scriptPath, inputPath];
        const child = child_process.spawn(pythonExecutable, args);
        ...
        child.on('close', async (code) => {
            ...
            if (code !== 0) {
                return res.status(500).json({ error: 'Analyze script failed', details: err || out });
            }
            try {
                const parsed = JSON.parse(out || '{}');
                return res.json(parsed);
            } catch (e) {
                return res.status(500).json({ error: 'Invalid JSON from analyze script', details: out });
            }
        });
    } catch (e) {
        res.status(500).json({ error: 'Server error in analyze-stl', details: e.message });
    }
});
// --- END Mesh Analysis Endpoint ---
