// Boller3D - Version 1.0.134 (2024-12-19) // Overlap robustness: PyVista fallback + voxel watertight rebuild
...
let toggleOverlapBtn; // Added for toggling overlap visibility
let toggleOverlapBboxBtn; // Added for toggling overlap bounding box
let toggleModelBtn; // Added for toggling main model visibility
let toggleFinalLogoStlBtn; // Added for toggling final logo STL visibility
let stlColorSelect; // Added for STL color control
let overlapColorSelect; // Added for overlap result color control
let finalLogoColorSelect; // Added for final logo result color control
let findOverlapBtn; // ADDED: Button for finding overlap between repair result and model
let makeFinalLogoStlBtn; // ADDED: Button for creating final logo STL
let make3DLogoBtn; // ADDED: Button for one-click full 3D logo pipeline
let mirrorLogoBtn; // ADDED: Button for mirroring final logo body
...
let overlapMesh = null; // ADDED: Reference for overlap result mesh
let overlapBboxHelper = null; // ADDED: Reference for overlap bounding box helper
let finalLogoStlMesh = null; // ADDED: Reference for final logo STL mesh
let mirroredFinalLogoStlMesh = null; // ADDED: Mirrored copy of final logo STL mesh
let axesHelper = null;
...
function init() {
    ...
    const BOLLER3D_VERSION = "1.0.138";

    // --- Initial Setup ---
    console.log(`%cBoller3D v${BOLLER3D_VERSION} - Critical fix for stray edges and zero-thickness corruption issues`, 'font-weight: bold;');
    bindDOMElements();
...
function bindDOMElements() {
    ...
    overlapColorSelect = document.getElementById('overlap-color'); // Added for overlap result color control
    finalLogoColorSelect = document.getElementById('final-logo-color'); // Added for final logo result color control
    findOverlapBtn = document.getElementById('find-overlap-btn'); // Added for finding overlap between repair result and model
    makeFinalLogoStlBtn = document.getElementById('make-final-logo-stl-btn'); // Added for final logo STL generation
    make3DLogoBtn = document.getElementById('make-3d-logo-btn'); // Added for one-click full 3D logo pipeline
    mirrorLogoBtn = document.getElementById('mirror-logo-btn'); // Added for mirroring final logo body
    ...
    toggleOverlapBtn = document.getElementById('toggle-overlap-btn'); // Added for toggling overlap visibility
    toggleOverlapBboxBtn = document.getElementById('toggle-overlap-bbox-btn'); // Added for toggling overlap bounding box
    toggleModelBtn = document.getElementById('toggle-model-btn'); // Added for toggling main model visibility
    toggleFinalLogoStlBtn = document.getElementById('toggle-final-logo-stl-btn'); // Added for toggling final logo STL visibility
    ...
    if (!viewerContainer || !modelSelect || !logoUploadInput || !previewLogoBtn || !doubleSidedCheckbox || 
        ...
        !stlThicknessInput || !logoThicknessDeltaInput || !stlColorSelect || !overlapColorSelect || !finalLogoColorSelect || !findOverlapBtn || !makeFinalLogoStlBtn || !make3DLogoBtn || !mirrorLogoBtn ||
        !downloadModelStlBtn || !repairBaseModelBtn || !reloadModelBtn || !debugModelsBtn || !toggleOverlapBtn || !toggleOverlapBboxBtn || !toggleModelBtn || !toggleFinalLogoStlBtn ) {
...
function populateColorSelectors() {
    ...
    PALETTE.forEach(color => {
        ...
        // Add color option to Overlap color selector as well
        if (overlapColorSelect) {
            const optionOverlap = optionLogo.cloneNode(true); // Clone for the Overlap color selector
            overlapColorSelect.appendChild(optionOverlap);
        }
        
        // Add color option to Final Logo color selector as well
        if (finalLogoColorSelect) {
            const optionFinalLogo = optionLogo.cloneNode(true); // Clone for the Final Logo color selector
            finalLogoColorSelect.appendChild(optionFinalLogo);
        }
    });
    ...
    if (overlapColorSelect) {
        overlapColorSelect.value = '#800080';  // Default Purple for Overlap
        styleColorSelect(overlapColorSelect);
    }
    if (finalLogoColorSelect) {
        finalLogoColorSelect.value = '#00FFFF';  // Default Cyan for Final Logo
        styleColorSelect(finalLogoColorSelect);
    }
...
function updateColorSelectorOptions(changedSelector, newValue) {
    const selectors = [modelColorSelect, logoColorSelect, stlColorSelect, overlapColorSelect, finalLogoColorSelect].filter(s => s); // Filter out nulls
...
function setupEventListeners() {
    ...
    // Add listener for Overlap Color Select
    if (overlapColorSelect) {
        overlapColorSelect.addEventListener('change', function(event) {
            const newOverlapColor = this.value;
            console.log(`Overlap color changed to: ${newOverlapColor}`);
            updateColorSelectorOptions(this, newOverlapColor);

            if (overlapMesh && overlapMesh.material) {
                overlapMesh.material.color.setStyle(newOverlapColor);
                overlapMesh.material.needsUpdate = true;
                console.log(`Updated overlap mesh color to: ${newOverlapColor}`);
            }
        });
    }

    // Add listener for Final Logo Color Select
    if (finalLogoColorSelect) {
        finalLogoColorSelect.addEventListener('change', function(event) {
            const newFinalLogoColor = this.value;
            console.log(`Final logo color changed to: ${newFinalLogoColor}`);
            updateColorSelectorOptions(this, newFinalLogoColor);

            if (finalLogoStlMesh && finalLogoStlMesh.material) {
                finalLogoStlMesh.material.color.setStyle(newFinalLogoColor);
                finalLogoStlMesh.material.needsUpdate = true;
                console.log(`Updated final logo STL color to: ${newFinalLogoColor}`);
            }
            if (mirroredFinalLogoStlMesh && mirroredFinalLogoStlMesh.material) {
                mirroredFinalLogoStlMesh.material.color.setStyle(newFinalLogoColor);
                mirroredFinalLogoStlMesh.material.needsUpdate = true;
                console.log(`Updated mirrored final logo STL color to: ${newFinalLogoColor}`);
            }
        });
    }
    ...
    // Add overlap toggle buttons
    if (toggleOverlapBtn) toggleOverlapBtn.addEventListener('click', function() {
        // Find overlap mesh in scene
        if (overlapMesh) {
            overlapMesh.visible = !overlapMesh.visible;
            console.log(`[TOGGLE OVERLAP] Visibility set to: ${overlapMesh.visible}`);
        } else {
            console.warn('No overlap mesh to toggle.');
        }
    });

    if (toggleFinalLogoStlBtn) toggleFinalLogoStlBtn.addEventListener('click', function() {
        if (finalLogoStlMesh) {
            finalLogoStlMesh.visible = !finalLogoStlMesh.visible;
            if (mirroredFinalLogoStlMesh) {
                mirroredFinalLogoStlMesh.visible = finalLogoStlMesh.visible;
            }
            console.log(`[TOGGLE FINAL LOGO STL] Visibility set to: ${finalLogoStlMesh.visible}`);
        } else {
            console.warn('No final logo STL mesh to toggle.');
        }
    });

    if (toggleOverlapBboxBtn) toggleOverlapBboxBtn.addEventListener('click', function() {
        // Find overlap mesh in scene
        if (overlapMesh) {
            // Toggle bounding box helper
            if (overlapBboxHelper) {
                scene.remove(overlapBboxHelper);
                overlapBboxHelper = null;
                console.log('[TOGGLE OVERLAP BBOX] Bounding box hidden');
            } else {
                const bbox = new THREE.Box3().setFromObject(overlapMesh);
                overlapBboxHelper = new THREE.Box3Helper(bbox, 0x800080); // Purple color
                scene.add(overlapBboxHelper);
                console.log('[TOGGLE OVERLAP BBOX] Bounding box shown');
            }
        } else {
            console.warn('No overlap mesh to toggle bounding box for.');
        }
    });

    // Add toggle model button
    if (toggleModelBtn) toggleModelBtn.addEventListener('click', function() {
        // Find main model in scene
        if (model) {
            model.visible = !model.visible;
            console.log(`[TOGGLE MODEL] Visibility set to: ${model.visible}`);
        } else {
            console.warn('No main model to toggle.');
        }
    });
...
function handleDownloadModelStl() {
    ...
    if (svgToStlGroup && svgToStlGroup.children.length > 0 && svgToStlGroup.children[0].geometry) {
    ...
    // --- Download ALL Boolean Results (if exist) ---
    // Check for any boolean result meshes in the scene
    const booleanResults = scene.children.filter(child => 
        child.type === 'Mesh' && 
        child.userData && 
        (child.userData.operationType === 'cut-logo' || 
         child.userData.operationType === 'cut-thin-logo' ||
         child.userData.operationType === 'subtraction' ||
         child.userData.operationType === 'cutout')
    );
    
    if (booleanResults.length > 0) {
        console.log(`[handleDownloadModelStl] Found ${booleanResults.length} boolean result(s), downloading...`);
        
        for (let i = 0; i < booleanResults.length; i++) {
            const booleanMesh = booleanResults[i];
            try {
                const exporter = new STLExporter();
                booleanMesh.updateMatrixWorld(true);
                const booleanStlString = exporter.parse(booleanMesh, { binary: false });
                
                // Generate filename
                const originalModelName = modelSelect.value ? modelSelect.value.replace(/\.stl$/i, '') : 'model';
                const logoBaseName = svgFileName ? svgFileName.replace(/\.[^.]*$/, '') : 'logo';
                const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
                const operationType = booleanMesh.userData?.operationType || 'boolean-result';
                const booleanFilename = `${originalModelName}_${logoBaseName}_${operationType}_${timestamp}.stl`;

                // Trigger download
                const blob = new Blob([booleanStlString], { type: 'text/plain' });
                saveAs(blob, booleanFilename);
                console.log(`[handleDownloadModelStl] Triggered download for boolean result ${i+1}: ${booleanFilename}`);
                downloadsInitiated++;
            } catch (error) {
                console.error(`[handleDownloadModelStl] Error exporting or downloading boolean result ${i+1}:`, error);
                alert(`Error downloading boolean result ${i+1}: ` + error.message);
            }
        }
    } else {
        console.log("[handleDownloadModelStl] No boolean results found to download.");
    }
    
    // Check for overlap result
    if (overlapMesh && overlapMesh.geometry) {
        console.log("[handleDownloadModelStl] Overlap result found, attempting download...");
        try {
            const exporter = new STLExporter();
            overlapMesh.updateMatrixWorld(true);
            const overlapStlString = exporter.parse(overlapMesh, { binary: false });
            
            // Generate filename
            const originalModelName = modelSelect.value ? modelSelect.value.replace(/\.stl$/i, '') : 'model';
            const logoBaseName = svgFileName ? svgFileName.replace(/\.[^.]*$/, '') : 'logo';
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const overlapFilename = `${originalModelName}_${logoBaseName}_overlap_${timestamp}.stl`;

            // Trigger download
            const blob = new Blob([overlapStlString], { type: 'text/plain' });
            saveAs(blob, overlapFilename);
            downloadsInitiated++;
            console.log(`[handleDownloadModelStl] Triggered download for overlap result: ${overlapFilename}`);
        } catch (error) {
            console.error("[handleDownloadModelStl] Error exporting or downloading overlap result:", error);
            alert("Error downloading the overlap result: " + error.message);
        }
    } else {
        console.log("[handleDownloadModelStl] No overlap result found to download.");
    }

    // 4) Download final logo STL result (if exists)
    if (finalLogoStlMesh && finalLogoStlMesh.geometry) {
        console.log("[handleDownloadModelStl] Final logo STL result found, attempting download...");
        try {
            const exporter = new STLExporter();
            finalLogoStlMesh.updateMatrixWorld(true);
            const finalLogoStlString = exporter.parse(finalLogoStlMesh, { binary: false });

            const logoBaseName = svgFileName ? svgFileName.replace(/\.[^/.]+$/, "") : "logo";
            const modelBaseName = currentModelFilename ? currentModelFilename.replace(/\.[^/.]+$/, "") : "model";
            const timestamp = Date.now();
            const finalLogoFilename = `${modelBaseName}_${logoBaseName}_final-logo-stl_${timestamp}.stl`;

            downloadStringAsFile(finalLogoStlString, finalLogoFilename, 'application/sla');
            downloadsInitiated++;
            console.log(`[handleDownloadModelStl] Triggered download for final logo STL: ${finalLogoFilename}`);
        } catch (error) {
            console.error("[handleDownloadModelStl] Error exporting or downloading final logo STL:", error);
        }
    } else {
        console.log("[handleDownloadModelStl] No final logo STL result found to download.");
    }
...
function debugBoundaryInfo() {
    // Check if we have boolean results to analyze instead
    let hasBooleanResults = false;
    for (const child of scene.children) {
        if (child.userData && child.userData.isBooleanResult) {
            hasBooleanResults = true;
            break;
        }
    }
    
    if (hasBooleanResults) {
        console.log("Boolean results detected - switching to geometry analysis...");
        debugBooleanResultGeometry();
        return;
    }
...
function getSvgToStlOptions() {
    // STL thickness is controlled in mm from the manual input field.
    // On each Logo STL operation, override with a 1.2x computed value.
    const currentThicknessMm = stlThicknessInput
        ? (parseFloat(stlThicknessInput.value) || 10.0)
        : 10.0;
    const typeDepth = currentThicknessMm * 1.2;
    const normalizedDepth = Math.max(0.1, parseFloat(typeDepth.toFixed(3)));
    if (stlThicknessInput) {
        stlThicknessInput.value = normalizedDepth.toString();
    }
    ...
    console.log(`[getSvgToStlOptions] Settings - Input Thickness: ${currentThicknessMm}mm, Final Depth: ${normalizedDepth}mm (input x 1.2), Precision: ${precision}, Color: ${objectColor}`);
    
    // Create options object
    return {
        typeDepth: normalizedDepth,
...
function matchLogoTransform() {
    ...
    // Preserve STL extrusion depth set by Logo STL operation (mm thickness control).
    // Do not remap depth from decal bounds, otherwise the thickness gets overridden.
    const scaleZ = 1;

    console.log(`[matchLogoTransform] Calculated Scales - XY: ${scaleXY.toFixed(3)}, Z: ${scaleZ.toFixed(3)} (depth preserved)`);
    ...
    // 8. Apply Translation DIRECTLY to the group position with small Z offset to prevent zero-thickness
    const zOffset = 0.1; // Small offset to prevent zero-thickness issues
    translation.z += zOffset;
    console.log(`[matchLogoTransform] Applied Z offset of ${zOffset}mm to prevent zero-thickness issues`);
    console.log(`[matchLogoTransform] Final Translation: (${translation.x.toFixed(2)}, ${translation.y.toFixed(2)}, ${translation.z.toFixed(2)})`);
...
function initSvgToStl() {
    ...
    if (findOverlapBtn) {
        ...
        console.warn("Find Overlap button not found");
    }
    
    if (makeFinalLogoStlBtn) {
        makeFinalLogoStlBtn.addEventListener('click', makeFinalLogoStl);
    } else {
        console.warn("Make Final Logo STL button not found");
    }

    if (make3DLogoBtn) {
        make3DLogoBtn.addEventListener('click', make3DLogoPipeline);
    } else {
        console.warn("Make 3D Logo button not found");
    }

    if (mirrorLogoBtn) {
        mirrorLogoBtn.addEventListener('click', mirrorFinalLogoBody);
    } else {
        console.warn("Mirror Logo button not found");
    }
    ...
    if (findOverlapBtn) findOverlapBtn.disabled = true; // Disabled until repair STL is pressed
    if (makeFinalLogoStlBtn) makeFinalLogoStlBtn.disabled = true; // Disabled until overlap+auto-repair completes
    if (make3DLogoBtn) make3DLogoBtn.disabled = false; // Always available for one-click flow
    if (mirrorLogoBtn) mirrorLogoBtn.disabled = true; // Enabled only after final logo STL exists
...
// --- One-click pipeline: Preview -> Logo STL -> Repair STL -> Find Overlap(+auto-repair) -> Make Final Logo STL
async function make3DLogoPipeline() {
    console.log("[Make 3D Logo] Starting one-click 3D logo pipeline...");
    ...
    const pipelineBtn = document.getElementById('make-3d-logo-btn');
    if (pipelineBtn) pipelineBtn.disabled = true;

    try {
        // Start from a clean state so waits don't pass on stale meshes
        if (finalLogoStlMesh) {
            scene.remove(finalLogoStlMesh);
            if (finalLogoStlMesh.geometry) finalLogoStlMesh.geometry.dispose();
            if (finalLogoStlMesh.material) finalLogoStlMesh.material.dispose();
            finalLogoStlMesh = null;
        }
        if (mirroredFinalLogoStlMesh) {
            scene.remove(mirroredFinalLogoStlMesh);
            if (mirroredFinalLogoStlMesh.geometry) mirroredFinalLogoStlMesh.geometry.dispose();
            if (mirroredFinalLogoStlMesh.material) mirroredFinalLogoStlMesh.material.dispose();
            mirroredFinalLogoStlMesh = null;
        }
        if (overlapMesh) {
            scene.remove(overlapMesh);
            if (overlapMesh.geometry) overlapMesh.geometry.dispose();
            if (overlapMesh.material) overlapMesh.material.dispose();
            overlapMesh = null;
        }
        if (mirrorLogoBtn) mirrorLogoBtn.disabled = true;

        // 1) Preview Logo
        const previousLogoMesh = logoMesh;
        uploadAndPreviewLogo();
        const previewReady = await waitForCondition(
            () => !!logoMesh && logoMesh !== previousLogoMesh && !!loadedLogoImage,
            25000,
            200
        );
        if (!previewReady) throw new Error("Preview Logo did not complete in time.");
        // showLogo() performs delayed auto-centering/max-scale; let that settle first
        const decalPlacementReady = await waitForCondition(
            () => !!lastPlacementPoint && !!lastPlacementNormal,
            5000,
            100
        );
        if (!decalPlacementReady) throw new Error("Logo placement normal/point did not become ready.");
        await new Promise(resolve => setTimeout(resolve, 900));
        console.log("[Make 3D Logo] Step 1/6 complete: Preview Logo");

        // 2) Logo STL using the same manual button flow
        const prevStlCount = (svgToStlGroup && svgToStlGroup.children) ? svgToStlGroup.children.length : 0;
        convertAndMatchLogoStl();
        const logoStlReady = await waitForCondition(
            () => !!svgToStlGroup && svgToStlGroup.children && svgToStlGroup.children.length > 0 && svgToStlGroup.children.length !== prevStlCount,
            20000,
            200
        );
        if (!logoStlReady) throw new Error("Logo STL generation did not complete in time.");
        // convertAndMatchLogoStl() schedules match at +500ms; wait for that to complete
        await new Promise(resolve => setTimeout(resolve, 700));
        console.log("[Make 3D Logo] Step 2/6 complete: Logo STL");

        // 3) Repair STL
        await repairStl();
        const repairedStlReady = await waitForCondition(
            () => !!svgToStlGroup && svgToStlGroup.children && svgToStlGroup.children.length === 1,
            20000,
            200
        );
        if (!repairedStlReady) throw new Error("Repair STL did not complete in time.");
        console.log("[Make 3D Logo] Step 3/6 complete: Repair STL");

        // 4) Find Overlap (+ automatic overlap repair)
        await findOverlap({ suppressAlert: true });
        const overlapReady = await waitForCondition(() => !!overlapMesh && !!overlapMesh.geometry, 20000, 200);
        if (!overlapReady) throw new Error("Find Overlap did not complete in time.");
        console.log("[Make 3D Logo] Step 4/5 complete: Find Overlap (+ auto-repair)");

        // 5) Make Final Logo STL
        await makeFinalLogoStl();
        const finalReady = await waitForCondition(() => !!finalLogoStlMesh && !!finalLogoStlMesh.geometry, 20000, 200);
        if (!finalReady) throw new Error("Make Final Logo STL did not complete in time.");
        console.log("[Make 3D Logo] Step 5/5 complete: Make Final Logo STL");

        console.log("[Make 3D Logo] Pipeline completed successfully.");
    } catch (error) {
        console.error("[Make 3D Logo] Pipeline failed:", error);
        alert(`Make 3D Logo failed: ${error.message}`);
    } finally {
...
/** Parse logo thickness delta (mm); 0 is valid — do not use `||` which treats 0 as missing. */
function parseLogoThicknessDeltaMm(inputEl) {
    if (!inputEl) return 0;
    ...
    try {
        ...
        */
        ...
        const response = await fetch('/api/repair-stl', {
            ...
            body: JSON.stringify({
                stlData: stlString,
                // Keep conservative to avoid sealing intended letter cavities.
                holeSize: 0.6
            }),
        });
        ...
        // Diagnostic: verify repaired output health (watertight/body count/volume)
        try {
            const analyzeResponse = await fetch('/api/analyze-stl', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ stlData: result.repairedStlData })
            });
            if (analyzeResponse.ok) {
                const a = await analyzeResponse.json();
                console.log(`[Repair STL][Health] watertight=${a.watertight}, volume=${a.volume}, bodies=${a.totalBodies}, faces=${a.faces}`);
            }
        } catch (diagErr) {
            console.warn("[Repair STL] Could not run repaired STL diagnostics:", diagErr);
        }
        ...
        */
        ...
        // STL from the server matches the export frame: world/scene coordinates. Parenting under
        // svgToStlGroup requires inverse(group.matrixWorld). Apply that to **geometry** (not the
        // mesh object) so the mesh matrix stays identity — avoids placement bugs from matrixWorld
        // vs .matrix mismatch and from Object3D.applyMatrix4 decomposition.
        svgToStlGroup.updateMatrixWorld(true);
        const inverseGroupWorld = new THREE.Matrix4().copy(svgToStlGroup.matrixWorld).invert();
        clearGroup(svgToStlGroup); // Clear existing children
        repairedGeometry.applyMatrix4(inverseGroupWorld);
        ...
        // Re-enable buttons
        if (repairStlBtn) repairStlBtn.disabled = false;
        if (findOverlapBtn) findOverlapBtn.disabled = false; // Enable Find Overlap button
    ...
    try {
        ...
        const response = await fetch(nodeApiUrl, {
            ...
            body: JSON.stringify({ 
                ...
                // Find Overlap should preserve original glyph topology for debugging/visual fidelity.
        ...
        // Remove previous Cut Logo result if it exists (allow multiple results)
        for (let i = scene.children.length - 1; i >= 0; i--) {
            const child = scene.children[i];
            if (child.userData && child.userData.isBooleanResult && child.userData.operationType === 'cut-logo') {
                console.log("Removing previous Cut Logo result mesh...");
                scene.remove(child);
                if (child.geometry) child.geometry.dispose();
                if (child.material) child.material.dispose();
            }
        ...
        const booleanResultMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color('#00FF00'), // Default to bright green for better visibility
            specular: 0x111111,
            shininess: 100,
            transparent: false,
            opacity: 1.0
        ...
        const booleanResultMesh = new THREE.Mesh(intersectedGeometry, booleanResultMaterial);
        // Store operation type for download filename and original position for Z offset
        booleanResultMesh.userData = { 
            operationType: 'cut-logo', 
            isBooleanResult: true,
            originalPosition: new THREE.Vector3() // Will be set after positioning
        };
        
        // Log the raw geometry details before any positioning
        const rawBbox = new THREE.Box3().setFromObject(booleanResultMesh);
        const rawSize = rawBbox.getSize(new THREE.Vector3());
        const rawCenter = rawBbox.getCenter(new THREE.Vector3());
        console.log(`[BOOLEAN RESULT CREATION] Raw geometry details:`);
        console.log(`  Raw Center: (${rawCenter.x.toFixed(2)}, ${rawCenter.y.toFixed(2)}, ${rawCenter.z.toFixed(2)})`);
        console.log(`  Raw Size: (${rawSize.x.toFixed(2)}, ${rawSize.y.toFixed(2)}, ${rawSize.z.toFixed(2)})`);
        console.log(`  Raw Z range: ${(rawCenter.z - rawSize.z/2).toFixed(2)} to ${(rawCenter.z + rawSize.z/2).toFixed(2)}`);
        console.log(`  Raw volume: ${(rawSize.x * rawSize.y * rawSize.z).toFixed(2)} cubic units`);
        
        // Check if the result is positioned correctly relative to the model
        if (model) {
            ...
            const modelTopZ = modelCenter.z + (modelSize.z / 2);
            const modelBottomZ = modelCenter.z - (modelSize.z / 2);
            
            console.log(`[BOOLEAN RESULT POSITIONING] Model reference:`);
            console.log(`  Model Z range: ${modelBottomZ.toFixed(2)} to ${modelTopZ.toFixed(2)}`);
            console.log(`  Result bottom vs Model top: ${(rawCenter.z - rawSize.z/2).toFixed(2)} vs ${modelTopZ.toFixed(2)}`);
            console.log(`  Result top vs Model bottom: ${(rawCenter.z + rawSize.z/2).toFixed(2)} vs ${modelBottomZ.toFixed(2)}`);
            
            // Check if result overlaps with model
            const resultBottomZ = rawCenter.z - rawSize.z/2;
            const resultTopZ = rawCenter.z + rawSize.z/2;
            
            if (resultBottomZ < modelTopZ && resultTopZ > modelBottomZ) {
                console.log(`  ⚠️ Result overlaps with model in Z direction`);
                const overlapAmount = Math.min(resultTopZ - modelBottomZ, modelTopZ - resultBottomZ);
                console.log(`  Overlap amount: ${overlapAmount.toFixed(2)}mm`);
            } else {
                console.log(`  ✅ Result does not overlap with model in Z direction`);
            }
        }
        
        // Apply fixed minimal Z offset (offset controls removed)
        let zOffset = 0.1;
        
        // Ensure minimum offset to prevent zero-thickness surfaces
        const minOffset = 0.1; // 0.1mm minimum offset
        if (Math.abs(zOffset) < minOffset) {
            zOffset = zOffset >= 0 ? minOffset : -minOffset;
            console.log(`[CUT LOGO] Applied minimum Z offset of ${zOffset}mm to prevent zero-thickness surfaces`);
        }
        if (zOffset !== 0.0) {
            console.log(`Positioning cut result with Z offset: ${zOffset}mm`);
            // Get model bounding box to position result relative to it
            if (model) {
                const modelBbox = new THREE.Box3().setFromObject(model);
                const modelCenter = modelBbox.getCenter(new THREE.Vector3());
                const modelSize = modelBbox.getSize(new THREE.Vector3());
                
                console.log(`[POSITION DEBUG] Model bounding box:`);
                console.log(`  Center: (${modelCenter.x.toFixed(2)}, ${modelCenter.y.toFixed(2)}, ${modelCenter.z.toFixed(2)})`);
                console.log(`  Size: (${modelSize.x.toFixed(2)}, ${modelSize.y.toFixed(2)}, ${modelSize.z.toFixed(2)})`);
                console.log(`  Top surface Z: ${(modelCenter.z + modelSize.z/2).toFixed(2)}`);
                
                // Position result so its outer surface (top) is at model top + offset
                const resultBbox = new THREE.Box3().setFromObject(booleanResultMesh);
                const resultSize = resultBbox.getSize(new THREE.Vector3());
                const resultCenter = resultBbox.getCenter(new THREE.Vector3());
                
                console.log(`[POSITION DEBUG] Result bounding box before positioning:`);
                console.log(`  Center: (${resultCenter.x.toFixed(2)}, ${resultCenter.y.toFixed(2)}, ${resultCenter.z.toFixed(2)})`);
                console.log(`  Size: (${resultSize.x.toFixed(2)}, ${resultSize.y.toFixed(2)}, ${resultSize.z.toFixed(2)})`);
                console.log(`  Top surface Z: ${(resultCenter.z + resultSize.z/2).toFixed(2)}`);
                
                // Position result so its outer surface (top) is at model top + offset
                // This ensures both results have their outer surfaces at the same position
                const modelTopZ = modelCenter.z + (modelSize.z / 2);
                const currentTopZ = resultCenter.z + (resultSize.z / 2);
                
                // Calculate target position: result top should be at model top + offset
                const targetTopZ = modelTopZ + zOffset;
                const moveAmount = targetTopZ - currentTopZ;
                const newZ = resultCenter.z + moveAmount;
                
                console.log(`[POSITION DEBUG] Outer surface positioning:`);
                console.log(`  Model top Z: ${modelTopZ.toFixed(2)}`);
                console.log(`  Current result top Z: ${currentTopZ.toFixed(2)}`);
                console.log(`  Target result top Z: ${targetTopZ.toFixed(2)} (model top + ${zOffset}mm offset)`);
                console.log(`  Moving result by: ${moveAmount.toFixed(2)}mm`);
                console.log(`  New center Z: ${newZ.toFixed(2)}`);
                
                booleanResultMesh.position.z = newZ - resultCenter.z;
                
                // Verify final position
                booleanResultMesh.updateMatrixWorld(true);
                const finalBbox = new THREE.Box3().setFromObject(booleanResultMesh);
                const finalCenter = finalBbox.getCenter(new THREE.Vector3());
                const finalSize = finalBbox.getSize(new THREE.Vector3());
                
                console.log(`[POSITION DEBUG] Final result bounding box:`);
                console.log(`  Center: (${finalCenter.x.toFixed(2)}, ${finalCenter.y.toFixed(2)}, ${finalCenter.z.toFixed(2)})`);
                console.log(`  Size: (${finalSize.x.toFixed(2)}, ${finalSize.y.toFixed(2)}, ${finalSize.z.toFixed(2)})`);
                console.log(`  Top surface Z: ${(finalCenter.z + finalSize.z/2).toFixed(2)}`);
                console.log(`  Actual offset from model top: ${((finalCenter.z + finalSize.z/2) - modelTopZ).toFixed(2)}mm`);
                
            } else {
                // Fallback: simple Z translation
                console.log(`Fallback: applying simple Z translation of ${zOffset}mm`);
                booleanResultMesh.position.z += zOffset;
            }
        }
        
        // Store the original position for Z offset calculations
        booleanResultMesh.userData.originalPosition.copy(booleanResultMesh.position);
...
function debugAvailableModels() {
    ...
    if (booleanResultMesh) {
        const boolBox = new THREE.Box3().setFromObject(booleanResultMesh);
        const boolSize = boolBox.getSize(new THREE.Vector3());
        const boolCenter = boolBox.getCenter(new THREE.Vector3());
        console.log('Boolean Result Mesh:', {
            ...
            geometry: booleanResultMesh.geometry ? {
                vertices: booleanResultMesh.geometry.attributes.position.count,
                faces: booleanResultMesh.geometry.index ? booleanResultMesh.geometry.index.count / 3 : 'unknown'
            } : 'no geometry'
        });
    } else {
    ...
    // Log overlap mesh
    if (overlapMesh) {
        const overlapBox = new THREE.Box3().setFromObject(overlapMesh);
        const overlapSize = overlapBox.getSize(new THREE.Vector3());
        const overlapCenter = overlapBox.getCenter(new THREE.Vector3());
        console.log('Overlap Mesh:', {
            operationType: overlapMesh.userData?.operationType,
            visible: overlapMesh.visible,
            position: overlapMesh.position,
            rotation: overlapMesh.rotation,
            scale: overlapMesh.scale,
            size: overlapSize,
            center: overlapCenter,
            geometry: overlapMesh.geometry ? {
                vertices: overlapMesh.geometry.attributes.position.count,
                faces: overlapMesh.geometry.index ? overlapMesh.geometry.index.count / 3 : 'unknown'
            } : 'no geometry'
        });
    } else {
        console.log('Overlap Mesh: Not available');
    }
    ...
    if (logoMesh) {
        ...
        console.log('Logo Decal Mesh:', {
            ...
            geometry: logoMesh.geometry ? {
                vertices: logoMesh.geometry.attributes.position.count,
                faces: logoMesh.geometry.index ? logoMesh.geometry.index.count / 3 : 'unknown'
            } : 'no geometry'
        });
    } else {
    ...
    // Log ALL meshes in scene (including boolean results)
    const allMeshes = scene.children.filter(child => child.type === 'Mesh');
    console.log(`All Meshes in Scene (${allMeshes.length}):`);
    ...
    allMeshes.forEach((mesh, index) => {
        ...
        console.log(`  Mesh ${index + 1}:`, {
            ...
            operationType: mesh.userData?.operationType || 'unknown',
            ...
            geometry: mesh.geometry ? {
                ...
                faces: mesh.geometry.index ? mesh.geometry.index.count / 3 : 'unknown'
            } : 'no geometry'
        });
    });
    ...
    // --- Bodies & Faces Analysis (server-backed) ---
    (async () => {
        const exporter = new STLExporter();
        async function analyzeMesh(name, object3d) {
            try {
                if (!object3d) return;
                object3d.updateMatrixWorld(true);
                const stl = exporter.parse(object3d, { binary: false });
                const resp = await fetch('/api/analyze-stl', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ stlData: stl })
                });
                if (!resp.ok) {
                    console.warn(`[ANALYZE] ${name}: server error ${resp.status}`);
                    return;
                }
                const data = await resp.json();
                console.group(`📦 ${name} Bodies Analysis`);
                console.log(`Watertight: ${data.watertight} | Faces: ${data.faces} | Vertices: ${data.vertices} | Volume: ${data.volume?.toFixed?.(6) ?? data.volume}`);
                console.log(`Total Bodies: ${data.totalBodies}`);
                if (Array.isArray(data.bodies)) {
                    data.bodies.forEach((b, i) => {
                        console.log(`  Body ${i + 1}: faces=${b.faces}, verts=${b.vertices}, watertight=${b.watertight}, volume=${(b.volume ?? 0).toFixed(6)}`);
                    });
                }
                console.groupEnd();
            } catch (e) {
                console.warn(`[ANALYZE] ${name}: failed:`, e);
            }
        }
        await analyzeMesh('Main Model', model);
        await analyzeMesh('Logo STL Group', svgToStlGroup);
        if (overlapMesh) await analyzeMesh('Overlap Mesh', overlapMesh);
    })();
...
async function cutThinLogoWithModel() {
    ...
    try {
        ...
        const thicknessDelta = parseLogoThicknessDeltaMm(thicknessDeltaInput);
        ...
        // Remove previous Cut Thin Logo result if it exists (allow multiple results)
        for (let i = scene.children.length - 1; i >= 0; i--) {
            const child = scene.children[i];
            if (child.userData && child.userData.isBooleanResult && child.userData.operationType === 'cut-thin-logo') {
                console.log("Removing previous Cut Thin Logo result mesh...");
                scene.remove(child);
                if (child.geometry) child.geometry.dispose();
                if (child.material) child.material.dispose();
            }
        ...
        // Create new boolean result mesh with its own material  
        const booleanResultMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color('#FF0000'), // Default to red for Cut Thin Logo
            specular: 0x111111,
            shininess: 100,
            transparent: false,
            opacity: 1.0
        ...
        const booleanResultMesh = new THREE.Mesh(thinIntersectedGeometry, booleanResultMaterial);
        // Store operation type for download filename and original position for Z offset
        booleanResultMesh.userData = { 
            operationType: 'cut-thin-logo', 
            isBooleanResult: true,
            originalPosition: new THREE.Vector3() // Will be set after positioning
        };
        ...
        if (Math.abs(zOffset) < minOffset) {
            zOffset = zOffset >= 0 ? minOffset : -minOffset;
            console.log(`[CUT THIN LOGO] Applied minimum Z offset of ${zOffset}mm to prevent zero-thickness surfaces`);
        }
        if (zOffset !== 0.0) {
            console.log(`Positioning thin cut result with Z offset: ${zOffset}mm`);
            // Get model bounding box to position result relative to it
            if (model) {
                const modelBbox = new THREE.Box3().setFromObject(model);
                const modelCenter = modelBbox.getCenter(new THREE.Vector3());
                ...
                // Position result so its bottom is offset from model's top
                const resultBbox = new THREE.Box3().setFromObject(booleanResultMesh);
                const resultSize = resultBbox.getSize(new THREE.Vector3());
                const resultCenter = resultBbox.getCenter(new THREE.Vector3());
                ...
                console.log(`  Center: (${resultCenter.x.toFixed(2)}, ${resultCenter.y.toFixed(2)}, ${resultCenter.z.toFixed(2)})`);
                console.log(`  Size: (${resultSize.x.toFixed(2)}, ${resultSize.y.toFixed(2)}, ${resultSize.z.toFixed(2)})`);
                console.log(`  Bottom surface Z: ${(resultCenter.z - resultSize.z/2).toFixed(2)}`);
...
// --- NEW Function: Repair Boolean Result Mesh ---
async function repairBooleanResultMesh() {
    console.log("Repairing boolean result mesh...");
    
    // Find the current boolean result mesh
    let booleanResultMesh = null;
    for (const child of scene.children) {
        if (child.userData && child.userData.isBooleanResult) {
            booleanResultMesh = child;
            break;
        }
    }
    
    if (!booleanResultMesh) {
        console.warn("No boolean result mesh found to repair.");
        alert("No boolean result mesh found to repair. Please create a boolean result first.");
        return;
    }
    
    console.log("Found boolean result mesh to repair:", booleanResultMesh);
    
    try {
        // Export the mesh to STL
        const stlExporter = new THREE.STLLoader.STLExporter();
        const stlString = stlExporter.parse(booleanResultMesh, { binary: false });
        
        console.log("Exported boolean result STL for repair (Text format)");
        
        // Send to repair endpoint
        const response = await fetch('/api/repair-stl', {
            method: 'POST',
            headers: {
        ...
        const repairedStlData = await response.text();
        console.log("Received repaired STL data from server");
        
        // Parse the repaired STL
        const loader = new THREE.STLLoader();
        const geometry = loader.parse(repairedStlData);
        
        // Create new material (preserve original material properties)
        const material = new THREE.MeshStandardMaterial({
            color: booleanResultMesh.material.color,
            transparent: booleanResultMesh.material.transparent,
            opacity: booleanResultMesh.material.opacity,
            side: THREE.DoubleSide
        });
        
        // Create new mesh with repaired geometry
        const repairedMesh = new THREE.Mesh(geometry, material);
        repairedMesh.userData = { ...booleanResultMesh.userData };
        repairedMesh.userData.isRepaired = true;
        
        // Position the repaired mesh at the same location
        repairedMesh.position.copy(booleanResultMesh.position);
        repairedMesh.rotation.copy(booleanResultMesh.rotation);
        repairedMesh.scale.copy(booleanResultMesh.scale);
        
        // Remove old mesh and add new one
        scene.remove(booleanResultMesh);
        scene.add(repairedMesh);
        
        // Log bounding box information
        const bbox = new THREE.Box3().setFromObject(repairedMesh);
        const size = bbox.getSize(new THREE.Vector3());
        const center = bbox.getCenter(new THREE.Vector3());
        console.log(`%c[REPAIRED BOOLEAN RESULT] Bounding Box - Center: (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)}), Size: (${size.x.toFixed(2)}, ${size.y.toFixed(2)}, ${size.z.toFixed(2)})`, 'font-weight: bold;');
        
        console.log("%c✅ Boolean result mesh repaired successfully.", 'font-weight: bold;');
        
        // Force render update
        if (renderer) renderer.render(scene, camera);
        
    } catch (error) {
        console.error("Error repairing boolean result mesh:", error);
        alert(`Boolean result repair failed: ${error.message}`);
    }
}

// --- Global function for console access ---
// This function can be called from the browser console to repair boolean results
window.repairBooleanResult = repairBooleanResultMesh;
window.debugBooleanResultGeometry = debugBooleanResultGeometry;
console.log("Global functions 'repairBooleanResult()' and 'debugBooleanResultGeometry()' available for console use");

// Global function for debugging boolean result geometry
function debugBooleanResultGeometry() {
    console.group("🔧 BOOLEAN RESULT GEOMETRY DEBUG");
    
    let foundAny = false;
    for (const child of scene.children) {
        if (child.userData && child.userData.isBooleanResult) {
            foundAny = true;
            console.group(`📊 ${child.userData.operationType.toUpperCase()} GEOMETRY ANALYSIS`);
            ...
            console.log(`  Volume: ${(size.x * size.y * size.z).toFixed(3)} cubic units`);
            
            if (geometry) {
                console.log("🔍 Geometry Analysis:");
                console.log(`  Vertices: ${geometry.attributes.position.count}`);
                console.log(`  Faces: ${geometry.index ? geometry.index.count / 3 : 'No index buffer'}`);
                console.log(`  Normals: ${geometry.attributes.normal ? geometry.attributes.normal.count : 'None'}`);
                
                // Analyze vertex positions for stray edges
                ...
                for (let i = 0; i < positions.length; i += 3) {
                    ...
                    // Check for vertices that are significantly outside the main body
                    const x = positions[i];
                    const y = positions[i + 1];
                    const distanceFromCenter = Math.sqrt(x*x + y*y);
                    if (distanceFromCenter > 60) { // More than 60mm from center
                        strayVertices.push({x: x.toFixed(3), y: y.toFixed(3), z: z.toFixed(3)});
                    }
                }
                
                console.log(`  Z Range: ${minZ.toFixed(3)} to ${maxZ.toFixed(3)} (span: ${(maxZ - minZ).toFixed(3)}mm)`);
                
                // Analyze Z distribution to detect potential issues
                const zValues = [];
                for (let i = 0; i < positions.length; i += 3) {
                ...
                // Check for unusual Z distributions
                const zPercentiles = {
                    p1: zValues[Math.floor(zValues.length * 0.01)],
                    p5: zValues[Math.floor(zValues.length * 0.05)],
                    p95: zValues[Math.floor(zValues.length * 0.95)],
                    p99: zValues[Math.floor(zValues.length * 0.99)]
                };
                
                console.log(`  Z Distribution: P1=${zPercentiles.p1.toFixed(3)}, P5=${zPercentiles.p5.toFixed(3)}, P95=${zPercentiles.p95.toFixed(3)}, P99=${zPercentiles.p99.toFixed(3)}`);
                
                // Check for potential stray edges (vertices far from main body)
                const mainBodyZRange = zPercentiles.p95 - zPercentiles.p5;
                const totalZRange = maxZ - minZ;
                const strayRatio = (totalZRange - mainBodyZRange) / totalZRange;
                
                if (strayRatio > 0.1) { // More than 10% of Z range is outside main body
                    console.warn(`⚠️  Potential stray edges detected: ${(strayRatio * 100).toFixed(1)}% of Z range outside main body`);
                } else {
                    console.log(`✅ Z distribution looks normal (${(strayRatio * 100).toFixed(1)}% potential stray)`);
                }
                
                if (strayVertices.length > 0) {
                    console.warn(`⚠️  Found ${strayVertices.length} potentially stray vertices:`);
                    strayVertices.slice(0, 10).forEach(v => console.log(`    (${v.x}, ${v.y}, ${v.z})`));
                    if (strayVertices.length > 10) console.log(`    ... and ${strayVertices.length - 10} more`);
                } else {
                    console.log("✅ No obvious stray vertices detected");
                }
                ...
                if (geometry.index) {
                    ...
                    if (degenerateFaces > 0) {
                        console.warn(`⚠️  Found ${degenerateFaces} degenerate faces`);
                    } else {
                        console.log("✅ No degenerate faces detected");
                    }
                }
            }
    ...
    if (!foundAny) {
        console.log("❌ No boolean result meshes found in scene");
    }
    
    // --- Extended: server-side body analysis for main model, overlap, and logo STL ---
    (async () => {
        const exporter = new STLExporter();
        async function analyzeMesh(name, object3d) {
            try {
...
// --- NEW Function: Find Overlap between Repair Result and Model ---
async function findOverlap(options = {}) {
    const { suppressAlert = false } = options;
    console.log("Performing overlap detection between repair result and model...");

    // UI Feedback: Show loading state
    const findOverlapBtn = document.getElementById('find-overlap-btn');
    const downloadBtn = document.getElementById('download-model-stl-btn');
    if (findOverlapBtn) findOverlapBtn.disabled = true;
    if (downloadBtn) downloadBtn.disabled = true;

    try {
        // 1. Pre-checks
        if (!svgToStlGroup || svgToStlGroup.children.length === 0) {
            console.warn("No logo STL mesh available for overlap detection.");
            throw new Error("Please create and repair a logo STL first.");
        }

        if (svgToStlGroup.children.length > 1) {
            console.warn(
                "[Find Overlap] Multiple meshes in logo group — exporting merged group STL."
            );
        }

        let hasLogoGeometry = false;
        svgToStlGroup.traverse((c) => {
            if (c.isMesh && c.geometry && c.geometry.attributes && c.geometry.attributes.position) {
                hasLogoGeometry = true;
            }
        });
        if (!hasLogoGeometry) {
            console.error("Could not get valid logo mesh geometry from group.");
            throw new Error("Internal error: Could not find valid logo mesh geometry.");
        }

        if (!model || !model.geometry) {
            console.error("No model mesh or geometry available for overlap detection.");
            throw new Error("Model is not loaded or has no geometry.");
        }

        console.log("Assuming logo STL is aligned. Proceeding with overlap detection...");
        // 2. Export Meshes to Text STL Strings
        const exporter = new STLExporter();
        ...
        // Export Logo (Apply world transform - group transform is needed)
        svgToStlGroup.updateMatrixWorld(true);
        const logoStlString = exporter.parse(svgToStlGroup, { binary: false });
        console.log(`Exported logo STL string (${(logoStlString.length / 1024).toFixed(1)} KB)`);

        // 3. Optional health diagnostics on Logo STL input before boolean
        try {
            const diagResp = await fetch('/api/analyze-stl', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ stlData: logoStlString })
            });
            if (diagResp.ok) {
                const diag = await diagResp.json();
                console.log(`[Find Overlap][Logo STL Health] watertight=${diag.watertight}, volume=${diag.volume}, bodies=${diag.totalBodies}, faces=${diag.faces}`);
            }
        } catch (diagErr) {
            console.warn("[Find Overlap] Logo STL health diagnostics unavailable:", diagErr);
        }

        // 4. Send to Node.js Server Endpoint for intersection
        const nodeApiUrl = '/api/intersect-stl-scripted';
        console.log(`Sending STL data to Node.js server at ${nodeApiUrl} for overlap detection...`);
        const response = await fetch(nodeApiUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            ...
            console.error("Server overlap detection failed:", errorData);
            let msg = `Server overlap detection failed: ${errorData.error || response.statusText}`;
            const det = typeof errorData.details === 'string' ? errorData.details.trim() : '';
            if (det) msg += ` (${det.slice(0, 900)})`;
            throw new Error(msg);
        }

        const result = await response.json();
        const overlapStlString = result.intersectedStlData;
        console.log("Received overlap STL data from server.");

        // 5. Parse Result and Create Overlap Mesh
        let overlapGeometry;
        try {
            overlapGeometry = loader.parse(overlapStlString);
            console.log("Overlap STL parsing completed successfully.");
        } catch (parseError) {
            console.error("Failed to parse overlap STL data:", parseError);
            throw new Error(`Failed to parse overlap STL: ${parseError.message}`);
        }

        // Check geometry validity
        if (!overlapGeometry || overlapGeometry.type !== 'BufferGeometry' || overlapGeometry.attributes.position.count === 0) {
            console.error("Parsed overlap geometry is invalid or empty.", overlapGeometry);
            throw new Error("Parsed overlap STL geometry is invalid or empty.");
        }
        
        overlapGeometry.computeVertexNormals();
        overlapGeometry.computeBoundingSphere();
        console.log("Overlap geometry parsed successfully.");

        // 6. Remove previous overlap mesh if it exists
        if (overlapMesh) {
            console.log("Removing previous overlap mesh...");
            scene.remove(overlapMesh);
            if (overlapMesh.geometry) overlapMesh.geometry.dispose();
            if (overlapMesh.material) overlapMesh.material.dispose();
            overlapMesh = null;
        ...
        // Remove previous overlap bounding box if it exists
        if (overlapBboxHelper) {
            scene.remove(overlapBboxHelper);
            overlapBboxHelper = null;
        }
        
        // 7. Create new overlap mesh with its own material
        const overlapMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color(overlapColorSelect ? overlapColorSelect.value : '#800080'), // Default Purple
            specular: 0x111111,
            shininess: 100,
            transparent: false,
            opacity: 1.0
        ...
        overlapMesh = new THREE.Mesh(overlapGeometry, overlapMaterial);
        overlapMesh.userData = { 
            operationType: 'overlap', 
            isOverlapResult: true
        };
        
        // Log the overlap geometry details
        const bbox = new THREE.Box3().setFromObject(overlapMesh);
        const size = bbox.getSize(new THREE.Vector3());
        const center = bbox.getCenter(new THREE.Vector3());
        console.log(`[OVERLAP RESULT] Geometry details:`);
        console.log(`  Center: (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)})`);
        console.log(`  Size: (${size.x.toFixed(2)}, ${size.y.toFixed(2)}, ${size.z.toFixed(2)})`);
        console.log(`  Volume: ${(size.x * size.y * size.z).toFixed(2)} cubic units`);
        console.log(`  Vertices: ${overlapGeometry.attributes.position.count}`);
        
        // Check if overlap exists
        if (size.x < 0.1 || size.y < 0.1 || size.z < 0.1) {
            console.warn("WARNING: Overlap result is very small - may indicate no significant overlap between logo and model");
        } else {
            console.log("SUCCESS: Significant overlap detected between logo and model");
        }
        
        scene.add(overlapMesh);
        console.log("Overlap mesh created and added to scene.");
        
        // 8. Update UI
        if (downloadBtn) downloadBtn.disabled = false; // Enable download for the result

        // Do not auto-run PyVista repair here — it can weld separate overlap shells into one blob.
        if (makeFinalLogoStlBtn) makeFinalLogoStlBtn.disabled = false;
        console.log("%cSUCCESS: Overlap detection completed (raw intersection).", 'font-weight: bold;');
        ...
        if (!suppressAlert) alert(`Overlap detection failed: ${error.message}`);
        throw error;
    } finally {
        if (findOverlapBtn) findOverlapBtn.disabled = false;
        if (renderer) renderer.render(scene, camera);
        if (controls) controls.update();
    }
}

// --- Repair Overlap Results (now called automatically from Find Overlap) ---
async function repairOverlap(options = {}) {
    const { suppressAlerts = false, disableButtonState = false, holeSize = 100.0 } = options;
    console.log("Performing repair on overlap result...");

    // UI Feedback: optional loading state (button removed from UI)
    const repairOverlapBtn = document.getElementById('repair-overlap-btn');
    if (disableButtonState && repairOverlapBtn) repairOverlapBtn.disabled = true;

    // 1. Pre-checks
    if (!overlapMesh || !overlapMesh.geometry) {
        console.warn("No overlap mesh available to repair.");
        if (!suppressAlerts) alert("Please create an overlap result first using 'Find Overlap'.");
        if (disableButtonState && repairOverlapBtn) repairOverlapBtn.disabled = false;
        return;
    }

    console.log("Repairing overlap geometry...");

    try {
        // 2. Export overlap mesh to STL string
        const exporter = new STLExporter();
        overlapMesh.updateMatrixWorld(true);
        const overlapStlString = exporter.parse(overlapMesh, { binary: false });
        console.log(`Exported overlap STL string (${(overlapStlString.length / 1024).toFixed(1)} KB)`);

        // 3. Send to Node.js Server Endpoint for repair
        const nodeApiUrl = '/api/repair-stl';
        console.log(`Sending overlap STL data to Node.js server at ${nodeApiUrl} for repair...`);
        const response = await fetch(nodeApiUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            const errorData = await response.json();
            console.error("Server overlap repair failed:", errorData);
            throw new Error(`Server overlap repair failed: ${errorData.error || response.statusText}`);
        }

        const result = await response.json();
        const repairedStlString = result.repairedStlData;
        console.log("Received repaired overlap STL data from server.");

        // 4. Parse Result and Update Overlap Mesh
        let repairedGeometry;
        try {
            repairedGeometry = loader.parse(repairedStlString);
            console.log("Repaired overlap STL parsing completed successfully.");
        } catch (parseError) {
            console.error("Failed to parse repaired overlap STL data:", parseError);
            throw new Error(`Failed to parse repaired overlap STL: ${parseError.message}`);
        }

        // Check geometry validity
        if (!repairedGeometry || repairedGeometry.type !== 'BufferGeometry' || repairedGeometry.attributes.position.count === 0) {
            console.error("Parsed repaired geometry is invalid or empty.", repairedGeometry);
            throw new Error("Parsed repaired overlap STL geometry is invalid or empty.");
        }
        
        repairedGeometry.computeVertexNormals();
        repairedGeometry.computeBoundingSphere();
        console.log("Repaired overlap geometry parsed successfully.");

        // 5. Update the existing overlap mesh with repaired geometry
        // Store the original material
        const originalMaterial = overlapMesh.material;
        
        // Remove old geometry
        if (overlapMesh.geometry) {
            overlapMesh.geometry.dispose();
        }
        
        // Update with repaired geometry
        overlapMesh.geometry = repairedGeometry;
        
        // Log the repaired geometry details
        const bbox = new THREE.Box3().setFromObject(overlapMesh);
        const size = bbox.getSize(new THREE.Vector3());
        const center = bbox.getCenter(new THREE.Vector3());
        console.log(`[REPAIRED OVERLAP RESULT] Geometry details:`);
        console.log(`  Center: (${center.x.toFixed(2)}, ${center.y.toFixed(2)}, ${center.z.toFixed(2)})`);
        console.log(`  Size: (${size.x.toFixed(2)}, ${size.y.toFixed(2)}, ${size.z.toFixed(2)})`);
        console.log(`  Volume: ${(size.x * size.y * size.z).toFixed(2)} cubic units`);
        console.log(`  Vertices: ${repairedGeometry.attributes.position.count}`);
        
        // Update bounding box helper if it exists
        if (overlapBboxHelper) {
            scene.remove(overlapBboxHelper);
            const newBbox = new THREE.Box3().setFromObject(overlapMesh);
            overlapBboxHelper = new THREE.Box3Helper(newBbox, 0x800080); // Purple color
            scene.add(overlapBboxHelper);
            console.log('[REPAIR OVERLAP] Updated bounding box helper');
        }
        
        console.log("Overlap mesh repaired and updated successfully.");
        
        // 6. Update UI
        if (makeFinalLogoStlBtn) makeFinalLogoStlBtn.disabled = false;
        console.log("SUCCESS: Overlap repair completed successfully.");

        // Force render update
        if (renderer) renderer.render(scene, camera);

    } catch (error) {
        console.error("Error during overlap repair:", error);
        if (!suppressAlerts) alert(`Overlap repair failed: ${error.message}`);
    } finally {
        // Re-enable button (if UI state handling was requested)
        if (disableButtonState && repairOverlapBtn) repairOverlapBtn.disabled = false;
        if (renderer) renderer.render(scene, camera);
        if (controls) controls.update();
    }
...
// --- NEW Function: Make Final Logo STL from repaired overlap ---
async function makeFinalLogoStl() {
    console.log("Generating final logo STL from repaired overlap and translated model...");

    if (!overlapMesh || !overlapMesh.geometry) {
        alert("Please run 'Find Overlap' first (it now performs overlap repair automatically).");
        return;
    }
    if (!model || !model.geometry) {
        alert("Model is not loaded or has no geometry.");
    ...
    const thicknessDelta = parseLogoThicknessDeltaMm(logoThicknessDeltaInput);
    const actionBtn = document.getElementById('make-final-logo-stl-btn');
    const downloadBtn = document.getElementById('download-model-stl-btn');
    ...
    try {
        ...
        // Create translated copy of the base model and move it in world +Z by thickness delta
        const translatedModel = model.clone();
        translatedModel.geometry = model.geometry.clone();
        translatedModel.position.z += thicknessDelta;
        translatedModel.updateMatrixWorld(true);

        overlapMesh.updateMatrixWorld(true);
        try {
            const overlapBodyCount = overlapMesh.geometry ? overlapMesh.geometry.clone().toNonIndexed().attributes.position.count : 0;
            console.log(`[MAKE FINAL LOGO] Overlap mesh vertex-count snapshot: ${overlapBodyCount}`);
        } catch (e) {
            console.warn('[MAKE FINAL LOGO] Could not capture overlap vertex-count snapshot.', e);
        }
        const translatedModelStl = exporter.parse(translatedModel, { binary: false });
        const repairedOverlapStl = exporter.parse(overlapMesh, { binary: false });

        const response = await fetch('/api/intersect-stl-scripted', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                modelStlData: translatedModelStl,
                logoStlData: repairedOverlapStl
            }),
        });
        ...
        const result = await response.json();
        const finalStlString = result.intersectedStlData;
        const finalGeometry = loader.parse(finalStlString);
        if (!finalGeometry || finalGeometry.type !== 'BufferGeometry' || finalGeometry.attributes.position.count === 0) {
            throw new Error("Final STL geometry is empty or invalid.");
        }
        finalGeometry.computeVertexNormals();
        finalGeometry.computeBoundingSphere();
        finalGeometry.computeBoundingBox();
        if (finalGeometry.boundingBox) {
            const finalSize = finalGeometry.boundingBox.getSize(new THREE.Vector3());
            console.log(`[MAKE FINAL LOGO] Final result size: (${finalSize.x.toFixed(3)}, ${finalSize.y.toFixed(3)}, ${finalSize.z.toFixed(3)})`);
            if (finalSize.z < 0.05) {
                console.warn(`[MAKE FINAL LOGO] WARNING: Final result has near-zero thickness in Z (${finalSize.z.toFixed(4)}).`);
            }
        }

        // Keep overlap untouched and create/update a separate final logo STL mesh
        if (finalLogoStlMesh) {
            scene.remove(finalLogoStlMesh);
            if (finalLogoStlMesh.geometry) finalLogoStlMesh.geometry.dispose();
            if (finalLogoStlMesh.material) finalLogoStlMesh.material.dispose();
        ...
        const finalLogoMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color(finalLogoColorSelect ? finalLogoColorSelect.value : '#00FFFF'),
            specular: 0x111111,
            shininess: 100,
            transparent: false,
            opacity: 1.0
        ...
        finalLogoStlMesh = new THREE.Mesh(finalGeometry, finalLogoMaterial);
        finalLogoStlMesh.userData = {
            operationType: 'final-logo-stl',
            isFinalLogoStl: true
        };
        scene.add(finalLogoStlMesh);
        if (mirrorLogoBtn) mirrorLogoBtn.disabled = false;

        console.log(`SUCCESS: Final logo STL generated using translated model offset ${thicknessDelta}mm in +Z.`);
        if (renderer) renderer.render(scene, camera);
    } catch (error) {
        console.error("Error generating final logo STL:", error);
        alert(`Make Final Logo STL failed: ${error.message}`);
    } finally {
...
function mirrorFinalLogoBody() {
    if (!finalLogoStlMesh || !finalLogoStlMesh.geometry) {
        alert("Please generate Final Logo STL first.");
        return;
    }
    if (!model || !model.geometry) {
    ...
    // Toggle behavior: if mirrored already exists, remove it and exit
    if (mirroredFinalLogoStlMesh) {
        scene.remove(mirroredFinalLogoStlMesh);
        if (mirroredFinalLogoStlMesh.geometry) mirroredFinalLogoStlMesh.geometry.dispose();
        if (mirroredFinalLogoStlMesh.material) mirroredFinalLogoStlMesh.material.dispose();
        mirroredFinalLogoStlMesh = null;
        if (renderer) renderer.render(scene, camera);
        console.log("Mirrored final logo body removed.");
        return;
    }

    // Clone final logo STL mesh
    const clonedGeometry = finalLogoStlMesh.geometry.clone();
    const clonedMaterial = finalLogoStlMesh.material.clone();
    mirroredFinalLogoStlMesh = new THREE.Mesh(clonedGeometry, clonedMaterial);

    // Copy source transform
    mirroredFinalLogoStlMesh.position.copy(finalLogoStlMesh.position);
    mirroredFinalLogoStlMesh.rotation.copy(finalLogoStlMesh.rotation);
    mirroredFinalLogoStlMesh.scale.copy(finalLogoStlMesh.scale);

    // Apply same mirroring operation as double-sided logo logic:
    // mirror around model center in X and Z planes
    model.geometry.computeBoundingBox();
    const modelCenter = model.geometry.boundingBox.getCenter(new THREE.Vector3());
    const sourcePosition = finalLogoStlMesh.position.clone();
    const offsetFromCenter = sourcePosition.clone().sub(modelCenter);
    const mirroredOffset = offsetFromCenter.clone();
    mirroredOffset.x *= -1;
    mirroredOffset.z *= -1;
    const mirroredPosition = modelCenter.clone().add(mirroredOffset);
    mirroredFinalLogoStlMesh.position.copy(mirroredPosition);

    const mirroredScale = mirroredFinalLogoStlMesh.scale.clone();
    mirroredScale.x *= -1;
    mirroredScale.z *= -1;
    mirroredFinalLogoStlMesh.scale.copy(mirroredScale);

    mirroredFinalLogoStlMesh.userData = {
        operationType: 'final-logo-stl-mirrored',
        isFinalLogoStl: true,
        isMirroredFinalLogo: true
    };

    scene.add(mirroredFinalLogoStlMesh);
    if (renderer) renderer.render(scene, camera);
    console.log("Mirrored final logo body created.");
}
